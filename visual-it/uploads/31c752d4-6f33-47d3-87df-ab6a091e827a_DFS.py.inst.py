...
def dfs_search(tree, start, goal):
    ...
    print(json.dumps({'event': 'assign', 'state': {
        'frontier': ([item[1] if isinstance(item, tuple) and len(item) >= 2 else item for item in stack] if 'stack' in locals() or 'stack' in globals() else []),
    ...
    while stack:
        ...
        print(json.dumps({'event': 'pop', 'state': {
            'frontier': list(stack) if 'stack' in locals() or 'stack' in globals() else [],
            'visited': list(visited) if 'visited' in locals() or 'visited' in globals() else (list(cost_so_far.keys()) if 'cost_so_far' in locals() or 'cost_so_far' in globals() else []),
